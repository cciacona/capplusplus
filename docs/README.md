# Documentation map

Cap++ documentation separates plans, observed facts, machine-checked coverage,
and project policy. A planned feature is not evidence, and a decoded format is
not evidence that the corresponding game system has been implemented.

## Project direction

- [Complete 1.0 parity roadmap](../ROADMAP.md)
- [Pre-engine architecture](architecture.md)
- [Architecture decision records](decisions/README.md)
- [Feature-parity ledger](parity.md)

## Original-format and executable research

- [Observed binary formats](formats.md)
- [Executable survey](executables.md)
- [Original file-loader contracts](loaders.md)
- [Map layout and terrain evidence](maps.md)
- [Terrain conversion and runtime initialization](terrain.md)
- [Simulation clock and random-number contracts](simulation-clock.md)
- [UI, layout-plan, and support-file formats](ui-resources.md)
- [Graphics catalog and palette profiles](graphics.md)
- [Audio formats and playback evidence](audio.md)

## Evidence, coverage, and verification

- [Validation results](validation.md)
- [Format completeness and safety gates](format-gates.md)
- [Original-content coverage](content-coverage.md)
- [Compatibility-quirks ledger](compatibility-quirks.md)
- [Controlled experiment records](experiments.md)
- [Repository and package gates](project-gates.md)

## Contribution and policy

- [Clean-room development policy](../CLEAN_ROOM.md)
- [Contributing](../CONTRIBUTING.md)
- [Reference projects and reuse policy](reference-projects.md)
- [Third-party notices](../THIRD_PARTY_NOTICES.md)
- [Security policy](../SECURITY.md)
- [Release history](../CHANGELOG.md)
